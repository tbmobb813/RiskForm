...
void main() {
  ...
  final writeLog = io.Platform.environment.containsKey('DIAGNOSTIC_LOG')
      ...
      : runDiag;
  final logDir =
      io.Platform.environment['DIAGNOSTIC_LOG_DIR'] ??
  ...
  testWidgets('JournalScreen finder diagnostics', (tester) async {
    ...
    await runZonedGuarded(
      () async {
        ...
        debugPrint(
          'Direct equality matches for $targetKey: ${foundByEquality.length}',
        );
        void printAncestorsTrimmed(Element e) {
          ...
          const max = 12;
          final toPrint = ancestors.length > max
              ? ancestors.sublist(0, max)
              : ancestors;
          debugPrint(
            '  Ancestor chain (top ${toPrint.length} of ${ancestors.length}):',
          );
          for (var a in toPrint) {
            debugPrint('    $a');
          }
          if (ancestors.length > max) {
            debugPrint('    ... ${ancestors.length - max} more');
          }
        ...
        for (final e in foundByEquality) {
          debugPrint(
            'Found element widget=${e.widget.runtimeType} key=${e.widget.key} mounted=${e.mounted}',
          );
        ...
        final byKeyC1NoSkip = find.byKey(
          const ValueKey('entry-c1'),
          skipOffstage: false,
        );
        final byKeyA1NoSkip = find.byKey(
          const ValueKey('entry-a1'),
          skipOffstage: false,
        );
        ...
        final byTextCycleNoSkip = find.textContaining(
          'Cycle 0',
          skipOffstage: false,
        );
        final byTextAssignmentNoSkip = find.text(
          'Assignment Event',
          skipOffstage: false,
        );
        final byTypeListTile = find.byType(ListTile);
        final byWidgetListTilePredicate = find.byWidgetPredicate(
          (w) => w.runtimeType.toString().contains('ListTile'),
        );
        final byElementKeyEq = find.byElementPredicate(
          (e) => e.widget.key == ValueKey('entry-c1'),
        );
    ...
    if (writeLog) {
      final name =
          'journal_ui_diagnostic_${DateTime.now().toIso8601String().replaceAll(':', '-')}.log';
