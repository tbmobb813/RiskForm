...
void main() {
  ...
  test(
    'Scenario A: Uptrend, high vol, deep liquidity produces parameter + widen recs',
    () async {
      ...
      final bundle = await engine.generate(
        context: ctx,
        regime: regime,
        vol: vol,
        liq: liq,
      ...
      expect(
        msgs.contains('delta') ||
            msgs.contains('raising target delta') ||
            msgs.contains('premium selling'),
        isTrue,
      );
    },
  );
  ...
  test(
    'Scenario B: Downtrend, high vol, thin liquidity yields defensive/risk-first recommendations',
    () async {
      final ctx = StrategyContext(
        healthScore: 40,
      ...
      final riskRecs = bundle.recommendations
          .where((r) => r.category == 'risk')
          .toList();
      expect(riskRecs.isNotEmpty, isTrue);
      expect(riskRecs.any((r) => r.priority == 1), isTrue);
      final msgs = riskRecs.map((r) => r.message).join(' || ');
      expect(
        msgs.contains('Reduce') ||
            msgs.contains('reduce') ||
            msgs.contains('Thin liquidity') ||
            msgs.contains('slippage'),
        isTrue,
      );
    },
  );
