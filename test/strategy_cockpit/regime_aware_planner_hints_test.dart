...
void main() {
  test(
    'generateHints recommends tighter delta on uptrend and applies backtest ranges',
    () {
      final ctx = StrategyContext(
        ...
        backtestComparison: BacktestSummary(
          bestConfig: {'dte': 30, 'delta': 0.18, 'width': 20},
        ),
      );
      ...
      expect(
        bundle.hints.any(
          (h) =>
              h.field == 'delta' && h.message.toLowerCase().contains('uptrend'),
        ),
        isTrue,
      );
  ...
  test('generateHints warns when delta outside constraints', () {
    ...
    expect(
      bundle.hints.any((h) => h.severity == 'danger' && h.field == 'delta'),
      isTrue,
    );
