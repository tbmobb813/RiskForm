...
void main() {
  ...
  test(
    'Scenario A narrative includes uptrend high-volatility phrase and actionable rec',
    () async {
      final ctx = StrategyContext(
        ...
        backtestComparison: BacktestSummary(
          bestConfig: {'delta': 0.05, 'dte': 30, 'width': 1},
        ),
      );

      final regime = MarketRegimeSnapshot(
        symbol: 'TST',
        trend: 'uptrend',
        volatility: 'high',
        liquidity: 'deep',
      ...
      final recs = await recEngine.generate(
        context: ctx,
        regime: regime,
        vol: vol,
        liq: liq,
      ...
      expect(
        narrative.bullets.any((b) => b.toLowerCase().contains('iv rank')),
        isTrue,
      );
  ...
  test(
    'Scenario B narrative flags downtrend high vol thin liquidity and recommends risk-first actions',
    () async {
      final ctx = StrategyContext(
        healthScore: 40,
      ...
      expect(
        narrative.bullets.any(
          (b) =>
              b.toLowerCase().contains('bid/ask') ||
              b.toLowerCase().contains('slippage'),
        ),
        isTrue,
      );
      expect(narrative.outlook.toLowerCase(), contains('actionable'));
      ...
      expect(
        recs.recommendations.any(
          (r) =>
              r.category == 'risk' ||
              r.message.toLowerCase().contains('reduce'),
        ),
        isTrue,
      );
    },
