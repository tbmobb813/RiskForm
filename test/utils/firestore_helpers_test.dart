...
import 'package:riskform/utils/firestore_helpers.dart' as fh;

void main() {
  test(
    'toFirestoreMap converts DateTime to Timestamp and preserves numbers',
    () {
      ...
      expect(
        (out['a'] as Timestamp).toDate().toUtc().millisecondsSinceEpoch,
      ...
      expect(
        (nested['x'] as Timestamp).toDate().toUtc().millisecondsSinceEpoch,
        equals(dt.millisecondsSinceEpoch),
      );
      ...
      expect(
        (list[0] as Timestamp).toDate().toUtc().millisecondsSinceEpoch,
        equals(dt.millisecondsSinceEpoch),
      );
