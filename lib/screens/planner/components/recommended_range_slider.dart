...
class RecommendedRangeSlider extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<RecommendedRangeSlider> createState() =>
      _RecommendedRangeSliderState();
}

class _RecommendedRangeSliderState extends ConsumerState<RecommendedRangeSlider>
  ...
  double _prevRecStart = 0,
      _prevRecEnd = 0,
      _prevBest = 0,
      _prevWeakStart = 0,
      _prevWeakEnd = 0;
  double _targetRecStart = 0,
      _targetRecEnd = 0,
      _targetBest = 0,
      _targetWeakStart = 0,
      _targetWeakEnd = 0;
  ...
  void didChangeDependencies() {
    ...
    final PlannerState state =
        widget.initialState ?? ref.read(plannerNotifierProvider);
    _applyBundle(state);
  }
  ...
  void _applyBundle(PlannerState state) {
    ...
    if (bundle != null && bundle.recommendedRanges.containsKey(widget.field)) {
      ...
      setState(() {
        _values = RangeValues(
          r.start.clamp(widget.min, widget.max),
          r.end.clamp(widget.min, widget.max),
        );
      });
    } else {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final PlannerState state =
        widget.initialState ?? ref.watch(plannerNotifierProvider);
    final bundle = state.hintsBundle;
    final recommended = bundle?.recommendedRanges[widget.field];
    final best = bundle?.bestPoints[widget.field];
    final weak = bundle?.weakRanges[widget.field];
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Recommended ${widget.field.toUpperCase()}',
                        ...
                        child: Builder(
                          builder: (context) {
                            ...
                            void startTransitionIfNeeded() {
                              if (_targetRecStart != newRecStart ||
                                  _targetRecEnd != newRecEnd ||
                                  _targetBest != newBest ||
                                  _targetWeakStart != newWeakStart ||
                                  _targetWeakEnd != newWeakEnd) {
                                final t = _controller.value;
                                double interp(double a, double b, double tt) =>
                                    a + (b - a) * tt;

                                final curRecStart = interp(
                                  _prevRecStart,
                                  _targetRecStart,
                                  t,
                                );
                                final curRecEnd = interp(
                                  _prevRecEnd,
                                  _targetRecEnd,
                                  t,
                                );
                                final curBest = interp(
                                  _prevBest,
                                  _targetBest,
                                  t,
                                );
                                final curWeakStart = interp(
                                  _prevWeakStart,
                                  _targetWeakStart,
                                  t,
                                );
                                final curWeakEnd = interp(
                                  _prevWeakEnd,
                                  _targetWeakEnd,
                                  t,
                                );
                            ...
                            return AnimatedBuilder(
                              ...
                              builder: (context, _) {
                                ...
                                final animRecStart = lerp(
                                  _prevRecStart,
                                  _targetRecStart,
                                );
                                final animRecEnd = lerp(
                                  _prevRecEnd,
                                  _targetRecEnd,
                                );
                                final animBest = lerp(_prevBest, _targetBest);
                                final animWeakStart = lerp(
                                  _prevWeakStart,
                                  _targetWeakStart,
                                );
                                final animWeakEnd = lerp(
                                  _prevWeakEnd,
                                  _targetWeakEnd,
                                );
                        ...
                        child: RangeSlider(
                          ...
                          onChanged: (rv) {
                            ...
                            final notifier = ref.read(
                              plannerNotifierProvider.notifier,
                            );
            ...
            const SizedBox(height: 8),
            Text(
              'Recommended range: ${values.start.toStringAsFixed(2)} — ${values.end.toStringAsFixed(2)}',
                ...
                children: [
                  if (weak != null) ...[
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: Colors.red.withAlpha((0.24 * 255).round()),
                  ...
                  if (best != null) ...[
                    ...
                    CustomPaint(
                      ...
                      painter: _DotLegendPainter(color: Colors.blue),
                    ),
                    const SizedBox(width: 6),
...
class _DotLegendPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _DotLegendPainter oldDelegate) =>
...
class _SliderOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (weakStart != null && weakEnd != null) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(rectW, const Radius.circular(4)),
        paint,
      );
      _drawDottedLine(
        canvas,
        Offset(l, size.height * 0.2),
        Offset(l, size.height * 0.8),
        color: Colors.red,
      );
      _drawDottedLine(
        canvas,
        Offset(r, size.height * 0.2),
        Offset(r, size.height * 0.8),
        color: Colors.red,
      );
    }
    ...
    if (recommendedStart != null && recommendedEnd != null) {
      final startFrac = ((recommendedStart! - min) / (max - min)).clamp(
        0.0,
        1.0,
      );
      ...
      if ((recommendedEnd! - recommendedStart!).abs() < 1e-6) {
        ...
        _drawDottedLine(
          canvas,
          Offset(markerX, size.height * 0.15),
          Offset(markerX, size.height * 0.85),
          color: Colors.blue,
        );
      }
    }
