...
class CockpitDebugScreen extends ConsumerWidget {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Test Data Creation
            _buildSection(context, 'Test Data Creation', [
              _buildButton(
                context,
            ...
            _buildSection(context, 'Pending Journal Testing', [
              _buildButton(
                context,
            ...
            // Current State
            _buildSection(context, 'Current Cockpit State', [
              _buildStatCard(
                'Discipline Score',
                '${state.discipline.currentScore}/100',
              ),
              _buildStatCard(
                'Clean Streak',
                '${state.discipline.cleanStreak} days',
              ),
              _buildStatCard(
                'Adherence Streak',
                '${state.discipline.adherenceStreak} days',
              ),
              _buildStatCard('Discipline Level', state.discipline.level.label),
              _buildStatCard(
                'Pending Journals',
                '${state.pendingJournals.length}',
              ),
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              '✅ All test data created! Navigate to cockpit to see it.',
            ),
  ...
  Future<void> _clearTestData(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'This will delete all journal entries and cockpit data. This cannot be undone.',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              '✅ Added pending journal. Try opening cockpit and clicking "New Trade Plan"',
            ),
  ...
  Future<void> _clearPendingJournals(
    BuildContext context,
    WidgetRef ref,
  ) async {
    try {
      ...
      for (final journal in state.pendingJournals) {
        await ref
            .read(cockpitControllerProvider.notifier)
            .removePendingJournal(journal.positionId);
      }
