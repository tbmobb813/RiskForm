...
class DefaultCockpitDataClient implements CockpitDataClient {
  ...
  DefaultCockpitDataClient(this._posService, [FirebaseFirestore? firestore])
    : _firestore = firestore ?? FirebaseFirestore.instance;

  @override
  Future<List<String>> fetchWatchlist(String uid) async {
    final wlDoc = await _firestore
        .collection('users')
        .doc(uid)
        .collection('cockpit')
        .doc('watchlist')
  ...
  Future<List<Map<String, dynamic>>> fetchPendingJournals(String uid) async {
    ...
    return (data?['journals'] as List<dynamic>?)
