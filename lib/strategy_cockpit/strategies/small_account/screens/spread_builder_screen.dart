...
class SpreadBuilderScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SpreadBuilderScreen> createState() =>
      _SpreadBuilderScreenState();
}
...
class LongStrikeSelector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final expiry = chain.expirations.firstWhere(
      (e) => e.expiry == selection.expiry,
    );
        ...
        ...expiry.calls.map((c) {
          ...
          return Card(
            child: ListTile(
              ...
              subtitle: Text(
                'Premium \$${c.premium.toStringAsFixed(2)} • Δ ${c.delta.toStringAsFixed(2)}',
              ),
              trailing: isSelected
...
class ShortStrikeSelector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final calls = expiry.calls
        .where((c) => c.contract.strike > selection.longLeg!.strike)
        .toList();
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Breakeven: ${(longLeg.strike + netDebit).toStringAsFixed(2)}',
            ),
          ],
        ),
      ),
            ...
            ? () {
                ...
                if (strategy != null) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Selection incomplete')),
                  );
                }
              }
            : null,
