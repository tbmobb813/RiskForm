...
class ScannerFiltersNotifier extends StateNotifier<ScannerFilters> {
  ...
  void setDteRange(int min, int max) =>
      state = state.copyWith(minDte: min, maxDte: max);
  void setDeltaRange(double min, double max) =>
      state = state.copyWith(minDelta: min, maxDelta: max);
  void setMinOpenInterest(int v) => state = state.copyWith(minOpenInterest: v);
  void setMaxBidAskSpread(double v) =>
      state = state.copyWith(maxBidAskSpread: v);
}

final scannerFiltersProvider =
    StateNotifierProvider<ScannerFiltersNotifier, ScannerFilters>((ref) {
...
final scannerResultsProvider =
    StateNotifierProvider<ScannerResultsNotifier, List<TradingStrategy>>((ref) {
