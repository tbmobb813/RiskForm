...
final strategyPersistenceServiceProvider = Provider<StrategyPersistenceService>(
  (ref) => StrategyPersistenceService(),
);

class StrategyPersistenceService {
  final FirebaseFirestore _db;

  StrategyPersistenceService([FirebaseFirestore? db])
    : _db = db ?? FirebaseFirestore.instance;

  Future<void> saveStrategy({
    required String uid,
    required PersistedStrategy strategy,
  }) async {
    await _db
        .collection('users')
        .doc(uid)
