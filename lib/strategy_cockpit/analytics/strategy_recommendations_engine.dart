...
class StrategyRecommendation {
  final String
  category; // "risk", "parameter", "regime", "discipline", "consistency"
...
class BacktestSummary {
  ...
  bestConfig; // lightweight container (e.g. {"dte":30, "delta":0.18})
...
StrategyRecommendationsBundle generateRecommendations(StrategyContext ctx) {
  ...
  if (best != null && best.isNotEmpty) {
    ...
    recs.add(
      StrategyRecommendation(category: 'parameter', message: msg, priority: 3),
    );
  }
  ...
  if (ctx.healthScore < 60) {
    recs.add(
      StrategyRecommendation(
        category: 'risk',
        message:
            'Health score ${ctx.healthScore} — reduce position size by 20–40%.',
        priority: 1,
      ),
    );
  }
  if (ctx.drawdown > 0.15) {
    recs.add(
      StrategyRecommendation(
        category: 'risk',
        message:
            'Drawdown ${(ctx.drawdown * 100).toStringAsFixed(0)}% — consider lowering trade frequency.',
        priority: 2,
      ),
    );
  }
  ...
  if (regime == 'uptrend') {
    recs.add(
      StrategyRecommendation(
        category: 'regime',
        message: 'Uptrend — favor premium selling when strength appears.',
        priority: 3,
      ),
    );
    ...
    recs.add(
      StrategyRecommendation(
        category: 'regime',
        message:
            'Downtrend — shift to defensive structures or reduce exposure.',
        priority: 2,
      ),
    );
    ...
    recs.add(
      StrategyRecommendation(
        category: 'regime',
        message: 'Sideways market — neutral income strategies preferred.',
        priority: 4,
      ),
    );
  }
  ...
  final lastDisc = ctx.disciplineTrend.isNotEmpty
      ? ctx.disciplineTrend.last
      : null;
  if (lastDisc != null && lastDisc < 60) {
    recs.add(
      StrategyRecommendation(
        category: 'discipline',
        message:
            'Recent discipline low ($lastDisc) — slow down and review journal notes.',
        priority: 1,
      ),
    );
  }
  ...
  if (ctx.disciplineTrend.length >= 3) {
    final changeCount = _countSignificantChanges(
      ctx.disciplineTrend,
      threshold: 10,
    );
    if (changeCount >= 2) {
      recs.add(
        StrategyRecommendation(
          category: 'discipline',
          message: 'Frequent adjustments detected — avoid intracycle changes.',
          priority: 2,
        ),
      );
    }
  ...
  if (varPnl > 0.05) {
    recs.add(
      StrategyRecommendation(
        ...
        message:
            'High PnL variance — consider narrowing width or tightening entries.',
        priority: 2,
      ),
    );
  }
  ...
  if (ctx.recentCycles.isNotEmpty) {
    final poorCycles = ctx.recentCycles
        .where((c) => c.disciplineScore < 60 || c.pnl < 0)
        .length;
    if (poorCycles >= (ctx.recentCycles.length / 2).ceil()) {
      recs.add(
        StrategyRecommendation(
          category: 'risk',
          message:
              'Multiple recent cycles underperforming — reduce size and review constraints.',
          priority: 1,
        ),
      );
    }
  ...
  return StrategyRecommendationsBundle(
  ...
  Future<StrategyRecommendationsBundle> generate({
    ...
    if (trend == 'uptrend') {
      ...
      if (bestDelta is num && bestDelta.toDouble() < 0.10) {
        out.add(
          StrategyRecommendation(
            category: 'parameter',
            message:
                'Uptrend — consider raising target delta to 0.15–0.20 for premium selling.',
            priority: 3,
          ),
        );
      } else {
        out.add(
          StrategyRecommendation(
            category: 'regime',
            message: 'Uptrend — favor premium selling with tighter deltas.',
            priority: 3,
          ),
        );
      }
      ...
      out.add(
        StrategyRecommendation(
          category: 'risk',
          message: 'Downtrend — reduce position size by 20–30%.',
          priority: 1,
        ),
      );
      ...
      if (bestWidth is num && bestWidth.toDouble() < 1.0) {
        out.add(
          StrategyRecommendation(
            category: 'parameter',
            message: 'Downtrend — widen structure width to absorb moves.',
            priority: 2,
          ),
        );
      }
      ...
      out.add(
        StrategyRecommendation(
          category: 'regime',
          message:
              'Sideways — neutral income and delta‑neutral structures preferred.',
          priority: 4,
        ),
      );
    }
    ...
    if (vol.ivRank >= 70) {
      out.add(
        StrategyRecommendation(
          category: 'risk',
          message: 'High volatility (IVR ≥70) — reduce size and widen width.',
          priority: 1,
        ),
      );
      ...
      for (var i = 0; i < out.length; i++) {
        if (out[i].category == 'risk') {
          out[i] = StrategyRecommendation(
            category: out[i].category,
            message: out[i].message,
            priority: _clampPriority(out[i].priority - 1),
          );
        }
      }
      ...
      out.add(
        StrategyRecommendation(
          category: 'parameter',
          message:
              'Low volatility (IVR ≤30) — consider tighter width and shorter DTE.',
          priority: 3,
        ),
      );
    }
    ...
    if (liqStr == 'thin') {
      out.add(
        StrategyRecommendation(
          category: 'risk',
          message: 'Thin liquidity — reduce size and avoid complex structures.',
          priority: 1,
        ),
      );
      if (liq.bidAskSpread > 0.05) {
        out.add(
          StrategyRecommendation(
            category: 'risk',
            message:
                'Wide bid/ask spread — expect slippage; prefer smaller sizes.',
            priority: 1,
          ),
        );
      }
      ...
      out.add(
        StrategyRecommendation(
          category: 'parameter',
          message: 'Deep liquidity — normal sizing acceptable.',
          priority: 4,
        ),
      );
    }
    ...
    final lastDisc = context.disciplineTrend.isNotEmpty
        ? context.disciplineTrend.last
        : null;
    if (trend == 'downtrend' &&
        vol.ivRank >= 70 &&
        lastDisc != null &&
