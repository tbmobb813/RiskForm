...
PlannerHintBundle generateHints(PlannerState state, StrategyContext ctx) {
  ...
  if (regime == 'uptrend') {
    ...
    hints.add(
      const PlannerHint(
        field: 'delta',
        message: 'Uptrend detected — tighter deltas favored (0.15–0.20).',
        severity: 'info',
      ),
    );
    ...
    if (state.delta > 0.25) {
      hints.add(
        PlannerHint(
          field: 'delta',
          message: 'Downtrend — delta > 0.25 is risky for this regime.',
          severity: 'warning',
        ),
      );
    }
    ...
    if (state.width < defensiveMinWidth) {
      hints.add(
        PlannerHint(
          field: 'width',
          message: 'Downtrend — consider wider width for defensive posture.',
          severity: 'info',
        ),
      );
      ranges['width'] = RangeValues(
        defensiveMinWidth,
        max(state.width, defensiveMinWidth),
      );
    }
    ...
    hints.add(
      const PlannerHint(
        field: 'type',
        message:
            'Sideways market — neutral income structures (IC, strangle) preferred.',
        severity: 'info',
      ),
    );
  }
  ...
  if (best != null && best.isNotEmpty) {
    ...
    hints.add(
      PlannerHint(
        field: 'backtest',
        message: 'Backtests favor: ${parts.join(', ')}.',
        severity: 'info',
      ),
    );
  }
      ...
      if (w is num && state.delta > w.toDouble()) {
        hints.add(
          PlannerHint(
            field: 'delta',
            message:
                'Backtests show weakness at delta > $w — consider tightening.',
            severity: 'warning',
          ),
        );
    ...
    if (weak.containsKey('dte')) {
      ...
      if (w is num && state.dte < w.toInt()) {
        hints.add(
          PlannerHint(
            field: 'dte',
            message: 'Backtests weak for DTE < $w in similar regimes.',
            severity: 'warning',
          ),
        );
  ...
  if (bw != null &&
      bw.toLowerCase().contains(ctx.currentRegime.toLowerCase())) {
    hints.add(
      PlannerHint(
        field: 'backtest',
        message: 'Backtests indicate weaknesses for current regime.',
        severity: 'danger',
      ),
    );
  }
    ...
    if (state.size > 5) {
      hints.add(
        PlannerHint(
          field: 'size',
          message:
              'Discipline slipping — reduce size relative to current (${state.size}).',
          severity: 'warning',
        ),
      );
    } else {
      hints.add(
        const PlannerHint(
          field: 'size',
          message: 'Discipline trend low — be conservative with sizing.',
          severity: 'info',
        ),
      );
    }
  ...
  if (ctx.disciplineTrend.length >= 3) {
    ...
    if (recent[2] < recent[1] && recent[1] < recent[0]) {
      hints.add(
        const PlannerHint(
          field: 'discipline',
          message: 'Discipline trending downward — consider reducing size.',
          severity: 'info',
        ),
      );
    }
  ...
  if (c.allowedDeltaRange != null && c.allowedDeltaRange!.length == 2) {
    ...
    if (state.delta < minD || state.delta > maxD) {
      hints.add(
        PlannerHint(
          field: 'delta',
          message: 'Delta ${state.delta} outside allowed range ($minD–$maxD).',
          severity: 'danger',
        ),
      );
    }
    ...
    if (state.dte < minT || state.dte > maxT) {
      hints.add(
        PlannerHint(
          field: 'dte',
          message:
              'DTE ${state.dte} outside allowed range (${minT.toInt()}–${maxT.toInt()}).',
          severity: 'warning',
        ),
      );
    }
  ...
  if (c.maxPositions <= 1 && state.size > 1) {
    hints.add(
      const PlannerHint(
        field: 'size',
        message: 'You are at max positions — cannot increase size further.',
        severity: 'danger',
      ),
    );
  }
  ...
  if (varPnl > 0.05) {
    hints.add(
      const PlannerHint(
        field: 'width',
        message: 'High PnL variance — recommend narrowing width.',
        severity: 'info',
      ),
    );
  ...
  return PlannerHintBundle(
    hints: hints,
    recommendedRanges: ranges,
    weakRanges: weakRanges,
