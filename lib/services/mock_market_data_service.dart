...
class MockMarketDataService implements MarketDataService {
  ...
  Stream<MarketPriceSnapshot> subscribePrice(
    ...
    return Stream<MarketPriceSnapshot>.periodic(
      dur,
      (_) =>
          _priceMap[symbol] ??
          MarketPriceSnapshot(
  ...
  Stream<MarketVolatilitySnapshot> subscribeVolatility(
    ...
    return Stream<MarketVolatilitySnapshot>.periodic(
      dur,
      (_) =>
          _volMap[symbol] ??
          MarketVolatilitySnapshot(
  ...
  Stream<MarketLiquiditySnapshot> subscribeLiquidity(
    ...
    final dur = interval ?? const Duration(seconds: 30);
    return Stream<MarketLiquiditySnapshot>.periodic(
      dur,
      (_) =>
          _liqMap[symbol] ??
          MarketLiquiditySnapshot(
  ...
  Stream<MarketRegimeSnapshot> subscribeRegime(
    ...
    return Stream<MarketRegimeSnapshot>.periodic(
      dur,
      (_) =>
          _regimeMap[symbol] ??
          MarketRegimeSnapshot(
