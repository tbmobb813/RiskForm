...
class DisciplineEngine {
  ...
  static int _scoreAdherence(
    ...
    if (plan['strike'] != null &&
        exec['strike'] != null &&
        plan['strike'] != exec['strike']) {
      score -= 15;
    }
    if (plan['expiration'] != null &&
        exec['expiration'] != null &&
        plan['expiration'] != exec['expiration']) {
      score -= 10;
    }
    if (plan['contracts'] != null &&
        exec['contracts'] != null &&
        plan['contracts'] != exec['contracts']) {
      score -= 15;
    }
  ...
  static int _scoreRisk(Map<String, dynamic> plan, Map<String, dynamic> exec) {
    ...
    final contractSize =
        (plan['contractSize'] as num?)?.toDouble() ??
        100.0; // shares per contract
    final execEntry = (exec['entryPrice'] as num?)?.toDouble();
    final contracts =
        (exec['contracts'] as num?)?.toDouble() ??
        (plan['contracts'] as num?)?.toDouble() ??
        0.0;
    ...
    if (planMaxRiskDollar == null &&
        planMaxRiskPct == null &&
        execRisk != null) {
      // execRisk interpreted as percent
      if (execRisk > 10.0) {
        score -= 10; // arbitrary penalty for high observed risk
      }
    }
